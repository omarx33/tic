...
$(document.body).on('change','form select[name=periodo]',function(){
  ...
  $(document).off('click','#listar');
  $(document).on('click','#listar',function(e){
    let periodo=$('#periodo').val();
    let dia=$('#dia').val();
    let tipo=$('#tipo_trabajador').val();
    if(periodo=='' || dia=='' || tipo==''){
      alertify.message('Los campos deben estar seleccionados');
    }else {
      $.post(baseurl+"Tareo/get_tareodiario",{periodo:periodo,dia:dia,tipo:tipo},function(data){
       if (data==0) {
            alert('El día no ha sido cerrado por parte del contrato');
            location.reload();
        }if (data==2) {
          alert('El día ya ha sido cerrado por Operaciones, solicite la apertura a Gestion Humana');
        }
        else {
